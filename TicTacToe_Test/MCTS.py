...
class MCTS:
    ...
    def vrniPotezo(self, game):

        self.root = Node(board=game)
        ...
        try:
            return self.pridobiNajboljsega(self.root)
        except:
            pass
    ...
    def select(self, node):
        ...
        while not node.isTerminal:
            if node.isFullyExpanded:
                node = self.pridobiNajboljsega(node)
            else:
    ...
    def expand(self, node):
        ...
        #         child_node = Node(move=move, parent=node, board=new_state)
    ...
    def pridobiNajboljsega(self, node):
        ...
        for child in node.children:
            ...
            if child.visits == 0:
                ...
                score = current_player * (exploit + self.exploration_weight * explore)

            # Preverjamo, če se je pojavil boljši rezultat
            if score > best_score:
                best_score = score
                best_child = [child]
            ...
            # Če je več enakih otrok, izberemo naključnega, da ne bi vedno izbrali istega otroka
